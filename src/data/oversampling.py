...
# function to output oversampling parquet 
def oversampling_dataset(clean_dataset, oversampling_data_path,target):
    ...
    oversampling_data.to_parquet(oversampling_data_path, index=False )

def oversampling_and_saved_data(config_path):
    """
    oversampling the dataset
    input: config path 
    output: save oversampling data files in output folder
    """
